...
from core.resources.replies import groups_list, keywords_list, no_groups_yet, no_keywords_yet, text_with_username, text_without_username


async def get_telegram_entity(client: TelegramClient, entity) -> User | Chat | Channel | None:
    try:
...
async def get_active_groups_list(active_groups: dict[int, Group]) -> str:
    ...
    for i, group in enumerate(active_groups, start=1):
        ...
        text += groups_list.format(i, active_groups[group].group_title, created_at.humanize(locale="ru"))
    if len(text) == 0:
        text = no_groups_yet
    return text
...
async def get_keywords_list(keywords: dict[int, Word]) -> str:
    ...
    for i, keyword in enumerate(keywords, start=1):
        ...
        text += keywords_list.format(i, keywords[keyword].keyword, created_at.humanize(locale="ru"))
    if len(text) == 0:
